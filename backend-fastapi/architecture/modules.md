...
12 modules, chacun structuré en un `APIRouter` FastAPI (`app/routers/<module>.py`), ses schémas Pydantic (`app/schemas/<module>.py`) et un service dédié (`app/services/<module>.py`). Toutes les routes sont préfixées par `/api`.
...
FastAPI génère et sert automatiquement la documentation de l'API à partir des schémas Pydantic et des annotations de type des routes :
