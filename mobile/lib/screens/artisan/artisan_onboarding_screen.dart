...
class _ArtisanOnboardingScreenState extends State<ArtisanOnboardingScreen> {
  ...
  void initState() {
    ...
    if (profile != null) {
      ...
      if (profile.hourlyRate != null) _hourlyRate.text = profile.hourlyRate.toString();
  ...
  Future<void> _save() async {
    if (_category == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Choisissez votre métier.')));
      return;
    }
    final years = int.tryParse(_experienceYears.text.trim());
    if (years == null || years < 0) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Indiquez un nombre d'années d'expérience valide.")));
      return;
    }
    ...
    try {
      await catalog.completeArtisanProfile({
        ...
        if (_hourlyRate.text.trim().isNotEmpty) 'hourlyRate': num.tryParse(_hourlyRate.text.trim()),
        if (_bio.text.trim().isNotEmpty) 'bio': _bio.text.trim(),
      });
      if (!mounted) return;
      messenger.showSnackBar(const SnackBar(content: Text('Profil professionnel enregistré.')));
      navigator.pop();
    } catch (_) {
      if (!mounted) return;
      messenger.showSnackBar(const SnackBar(content: Text("Impossible d'enregistrer votre profil. Réessayez.")));
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(title: Text(isEditing ? 'Profil professionnel' : 'Compléter mon profil')),
      body: ListView(
        padding: const EdgeInsets.fromLTRB(20, 8, 20, 32),
        children: [
          Text(
            isEditing ? 'Mettez à jour vos informations de métier.' : 'Ces informations permettent aux clients de vous trouver.',
          ...
          DropdownButtonFormField<Category>(
            value: _category,
            items: [for (final category in categories) DropdownMenuItem(value: category, child: Text(category.name))],
            onChanged: (value) => setState(() => _category = value),
            decoration: const InputDecoration(hintText: 'Sélectionnez votre métier'),
          ),
          const SizedBox(height: 20),
          Text('ANNÉES D\'EXPÉRIENCE', style: Theme.of(context).textTheme.labelSmall),
          const SizedBox(height: 8),
          TextField(
          ...
          const SizedBox(height: 20),
          Text('TARIF HORAIRE (FCFA, OPTIONNEL)', style: Theme.of(context).textTheme.labelSmall),
          const SizedBox(height: 8),
          TextField(
          ...
          const SizedBox(height: 20),
          Text('BIO (OPTIONNEL)', style: Theme.of(context).textTheme.labelSmall),
          const SizedBox(height: 8),
          TextField(
            controller: _bio,
            minLines: 3,
            maxLines: 5,
            decoration: const InputDecoration(hintText: 'Présentez votre expérience et vos spécialités.'),
          ),
